            ...
            <div class="card-body p-4 p-md-5">
              <form @submit.prevent="loginHandyman">
                ...
                <div class="text-end mb-4">
                  <router-link to="/handyman/forgot-password" class="auth-link">
                    Forgot Password?
                  </router-link>
                </div>
              ...
              <p class="text-center auth-footer-text mt-4 mb-0">
                No account yet?
                <router-link to="/handyman/register" class="auth-link">Register here</router-link>
              </p>
            </div>
          </div>
        </div>
...
const loginHandyman = async () => {
  try {
    loading.value = true;
    const response = await api.post("/handymen/login", form);
    ...
    notyf.error(err.response?.data?.error || err.message || "Failed to login handyman.");
  } finally {
    loading.value = false;
  }
};
