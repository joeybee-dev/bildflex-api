    ...
    <div class="container">
      <div class="card auth-card border-0 shadow-sm">
        <div class="auth-header text-center">
          <h2 class="auth-title mb-2">Admin Handymen</h2>
          <p class="auth-subtitle mb-0">Manage registered handyman accounts.</p>
        </div>
        ...
        <div class="card-body p-4 p-md-5">
          <div class="mb-4">
            <input
              ...
              type="text"
              class="form-control"
              placeholder="Search handymen by business name or email"
            />
          </div>
          ...
          <div v-else-if="filteredHandymen.length === 0" class="empty-state text-center py-5">
            <h5 class="mb-2">No handymen found</h5>
          ...
          <div v-else class="table-responsive">
            <table class="table align-middle admin-table">
              ...
              <tbody>
                <tr v-for="handyman in filteredHandymen" :key="handyman._id">
                  <td>{{ handyman.businessName || "-" }}</td>
                  <td>{{ handyman.email || "-" }}</td>
                  <td>{{ handyman.city || "-" }}</td>
                  <td>{{ handyman.isActive === false ? "Inactive" : "Active" }}</td>
                  <td class="text-end">
                    <router-link :to="`/admin/handymen/${handyman._id}`" class="btn login-btn btn-sm px-3">
                      View
                    </router-link>
                  </td>
                </tr>
...
const filteredHandymen = computed(() => {
  ...
  return handymen.value.filter((handyman) =>
    `${handyman.businessName || ""} ${handyman.email || ""}`
      .toLowerCase()
      .includes(term)
  );
});
...
const fetchHandymen = async () => {
  try {
    loading.value = true;
    const response = await api.get("/admin/handymen");
