    ...
    <div class="container">
      <div class="card auth-card border-0 shadow-sm">
        <div class="auth-header text-center">
          <h2 class="auth-title mb-2">Admin Designers</h2>
          <p class="auth-subtitle mb-0">Manage registered designer accounts.</p>
        </div>
        ...
        <div class="card-body p-4 p-md-5">
          <div class="mb-4">
            <input
              ...
              type="text"
              class="form-control"
              placeholder="Search designers by business name or email"
            />
          </div>
          ...
          <div v-else-if="filteredDesigners.length === 0" class="empty-state text-center py-5">
            <h5 class="mb-2">No designers found</h5>
          ...
          <div v-else class="table-responsive">
            <table class="table align-middle admin-table">
              ...
              <tbody>
                <tr v-for="designer in filteredDesigners" :key="designer._id">
                  <td>{{ designer.businessName || "-" }}</td>
                  <td>{{ designer.email || "-" }}</td>
                  <td>{{ designer.city || "-" }}</td>
                  <td>{{ designer.isActive === false ? "Inactive" : "Active" }}</td>
                  <td class="text-end">
                    <router-link :to="`/admin/designers/${designer._id}`" class="btn login-btn btn-sm px-3">
                      View
                    </router-link>
                  </td>
                </tr>
...
const filteredDesigners = computed(() => {
  ...
  return designers.value.filter((designer) =>
    `${designer.businessName || ""} ${designer.email || ""}`
      .toLowerCase()
      .includes(term)
  );
});
