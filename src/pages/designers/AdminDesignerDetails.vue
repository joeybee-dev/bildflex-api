    ...
    <div class="container">
      <div class="card auth-card border-0 shadow-sm">
        <div class="auth-header text-center">
          <h2 class="auth-title mb-2">Admin Designer Details</h2>
          <p class="auth-subtitle mb-0">Review designer account information.</p>
        </div>
          ...
          <div v-else class="row g-4">
            ...
            <div class="col-12 col-md-6">
              <div class="info-box">
                ...
                <div>{{ designer.contactPerson || "-" }}</div>
              </div>
            </div>
            ...
            <div class="col-12 col-md-6">
              <div class="info-box">
                ...
                <div>{{ designer.isActive === false ? "Inactive" : "Active" }}</div>
              </div>
            </div>
            ...
            <div class="col-12 col-md-6">
              <div class="info-box">
                ...
                <div>{{ designer.specialization || "-" }}</div>
              </div>
            </div>
            ...
            <div class="col-12">
              <div class="info-box info-box-lg">
                ...
                <div>{{ designer.aboutBusiness || "-" }}</div>
              </div>
            </div>

            <div class="col-12">
              <router-link to="/admin/designers" class="btn login-btn">Back to Admin Designers</router-link>
            </div>
          </div>
        </div>
      </div>  
...
const fetchDesigner = async () => {
  try {
    loading.value = true;
    const response = await api.get(`/admin/designers/${route.params.id}`);
    Object.assign(designer, response.data?.designer || response.data || {});
  } catch (err) {
    notyf.error(err.response?.data?.error || "Failed to load designer details.");
  } finally {
    loading.value = false;
  }
};

onMounted(fetchDesigner);
</script>
