        ...
        <div class="col-12 col-md-8 col-lg-5">
          <div class="card auth-card border-0 shadow-sm">
            <div class="auth-header text-center">
              ...
              <p class="auth-subtitle mb-0">Upload or remove your designer profile photo.</p>
            </div>

            <div class="card-body p-4 p-md-5 text-center">
              <div class="profile-image-wrapper mx-auto mb-4">
                <img :src="previewImage" alt="Designer Photo" class="profile-image" />
              </div>
              ...
              <div class="d-grid gap-2">
                ...
                <router-link to="/designer/profile" class="btn btn-outline-secondary custom-outline-btn">
                  Back to Profile
                </router-link>
              </div>
            </div>
...
const fetchProfile = async () => {
  try {
    ...
    currentPhoto.value = response.data?.designer?.profilePhoto || "";
  } catch {}
};
...
const uploadPhoto = async () => {
  ...
  try {
    ...
    const response = await api.patch("/designers/update-photo", formData, {
      headers: { "Content-Type": "multipart/form-data" }
    });

    currentPhoto.value = response.data?.designer?.profilePhoto || response.data?.profilePhoto || "";
...
const removePhoto = async () => {
  try {
    loading.value = true;
    const response = await api.delete("/designers/update-photo");
    currentPhoto.value = "";
    selectedFile.value = null;
