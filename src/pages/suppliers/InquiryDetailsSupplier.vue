    ...
    <div class="container">
      <div class="card auth-card border-0 shadow-sm">
        <div class="auth-header text-center">
          ...
          <p class="auth-subtitle mb-0">Review the full inquiry sent to your supplier account.</p>
        </div>
        ...
        <div class="card-body p-4 p-md-5">
          ...
          <div v-else class="row g-4">
            ...
            <div class="col-12">
              <router-link to="/supplier/inquiries" class="btn login-btn">Back to My Inquiries</router-link>
            </div>
          </div>
        </div>
      </div>
...
const fetchInquiry = async () => {
  try {
    loading.value = true;
    const response = await api.get(`/suppliers/inquiries/${route.params.id}`);
