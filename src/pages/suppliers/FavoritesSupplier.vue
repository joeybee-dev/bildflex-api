    ...
    <div class="container">
      <div class="card auth-card border-0 shadow-sm">
        <div class="auth-header text-center">
          ...
          <p class="auth-subtitle mb-0">Saved suppliers, products, or customer references.</p>
        </div>
          ...
          <div v-else class="row g-4">
            <div v-for="favorite in favorites" :key="favorite._id" class="col-12 col-md-6 col-lg-4">
              <div class="content-card h-100">
                ...
                <p class="content-meta text-center mb-1">{{ favorite.category || favorite.type || "-" }}</p>
                <p class="content-meta text-center mb-3">{{ favorite.city || "-" }}</p>

                <div class="d-grid gap-2">
                  <router-link
                    :to="favorite._id ? `/suppliers/${favorite._id}` : '/suppliers'"
                    class="btn login-btn"
                  >
                    View Details
                  </router-link>
...
const fetchFavorites = async () => {
  try {
    loading.value = true;
    const response = await api.get("/suppliers/favorites");
...
const removeFavorite = async (favoriteId) => {
  try {
    const response = await api.delete(`/suppliers/favorites/${favoriteId}`);
