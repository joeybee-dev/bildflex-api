    ...
    <div class="container">
      <div class="card auth-card border-0 shadow-sm">
        <div class="auth-header text-center">
          <h2 class="auth-title mb-2">Admin Suppliers</h2>
          <p class="auth-subtitle mb-0">Manage registered supplier accounts.</p>
        </div>
        ...
        <div class="card-body p-4 p-md-5">
          <div class="mb-4">
            <input
              ...
              type="text"
              class="form-control"
              placeholder="Search suppliers by business name or email"
            />
          </div>
          ...
          <div v-else-if="filteredSuppliers.length === 0" class="empty-state text-center py-5">
            <h5 class="mb-2">No suppliers found</h5>
          ...
          <div v-else class="table-responsive">
            <table class="table align-middle admin-table">
              ...
              <tbody>
                <tr v-for="supplier in filteredSuppliers" :key="supplier._id">
                  <td>{{ supplier.businessName || "-" }}</td>
                  <td>{{ supplier.email || "-" }}</td>
                  <td>{{ supplier.city || "-" }}</td>
                  <td>{{ supplier.isActive === false ? "Inactive" : "Active" }}</td>
                  <td class="text-end">
                    <router-link :to="`/admin/suppliers/${supplier._id}`" class="btn login-btn btn-sm px-3">
                      View
                    </router-link>
                  </td>
                </tr>
