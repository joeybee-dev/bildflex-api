    ...
    <div class="container">
      <div class="card auth-card border-0 shadow-sm">
        <div class="auth-header text-center">
          <h2 class="auth-title mb-2">Supplier Details</h2>
          <p class="auth-subtitle mb-0">View supplier profile and company information.</p>
        </div>
        ...
        <div class="card-body p-4 p-md-5">
          ...
          <div v-else class="row g-4 align-items-start">
            <div class="col-12 col-md-4 text-center">
              <img :src="supplier.profilePhoto || fallbackImage" class="profile-image-lg" alt="Supplier" />
            </div>
              ...
              <div class="row g-3">
                ...
                <div class="col-12">
                  <div class="info-box info-box-lg">
                    ...
                    <div>{{ supplier.aboutBusiness || "-" }}</div>
                  </div>
                </div>
              </div>

              <div class="mt-4">
                <router-link to="/suppliers" class="btn login-btn">Back to Suppliers</router-link>
              </div>
            </div>
          </div>
        </div>
...
const fetchSupplier = async () => {
  try {
    ...
    notyf.error(err.response?.data?.error || "Failed to load supplier details.");
  } finally {
    loading.value = false;
  }
};
