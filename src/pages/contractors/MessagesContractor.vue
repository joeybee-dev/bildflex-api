    ...
    <div class="container">
      <div class="card auth-card border-0 shadow-sm">
        <div class="auth-header text-center">
          <h2 class="auth-title mb-2">Booking Details</h2>
          <p class="auth-subtitle mb-0">Review booking details for your contractor service.</p>
        </div>
          ...
          <div v-else class="row g-4">
            <div class="col-12 col-md-6">
              <div class="info-box">
                ...
                <div>{{ booking.serviceTitle || booking.title || "-" }}</div>
              </div>
            </div>
            ...
            <div class="col-12 col-md-6">
              <div class="info-box">
                ...
                <div>{{ booking.customerName || booking.userName || "-" }}</div>
              </div>
            </div>
            ...
            <div class="col-12 col-md-6">
              <div class="info-box">
                ...
                <div>{{ formatDate(booking.bookingDate || booking.date) }}</div>
              </div>
            </div>
            ...
            <div class="col-12">
              <div class="info-box info-box-lg">
                ...
                <div>{{ booking.notes || booking.description || "-" }}</div>
              </div>
            </div>

            <div class="col-12">
              <router-link to="/contractor/bookings" class="btn login-btn">Back to My Bookings</router-link>
            </div>
          </div>
        </div>
      </div>  
...
const fetchBooking = async () => {
  try {
    loading.value = true;
    const response = await api.get(`/contractors/bookings/${route.params.id}`);
    Object.assign(booking, response.data?.booking || response.data || {});
  } catch (err) {
    notyf.error(err.response?.data?.error || "Failed to load booking details.");
  } finally {
    loading.value = false;
  }
};
