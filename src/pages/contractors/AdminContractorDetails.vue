    ...
    <div class="container">
      <div class="card auth-card border-0 shadow-sm">
        <div class="auth-header text-center">
          <h2 class="auth-title mb-2">Admin Contractors</h2>
          <p class="auth-subtitle mb-0">Manage registered contractor accounts.</p>
        </div>
        ...
        <div class="card-body p-4 p-md-5">
          <div class="mb-4">
            <input
              ...
              type="text"
              class="form-control"
              placeholder="Search contractors by business name or email"
            />
          </div>
          ...
          <div v-else-if="filteredContractors.length === 0" class="empty-state text-center py-5">
            <h5 class="mb-2">No contractors found</h5>
          ...
          <div v-else class="table-responsive">
            <table class="table align-middle admin-table">
              ...
              <tbody>
                <tr v-for="contractor in filteredContractors" :key="contractor._id">
                  <td>{{ contractor.businessName || "-" }}</td>
                  <td>{{ contractor.email || "-" }}</td>
                  <td>{{ contractor.city || "-" }}</td>
                  <td>{{ contractor.isActive === false ? "Inactive" : "Active" }}</td>
                  <td class="text-end">
                    <router-link :to="`/admin/contractors/${contractor._id}`" class="btn login-btn btn-sm px-3">
                      View
                    </router-link>
                  </td>
                </tr>
...
const filteredContractors = computed(() => {
  ...
  return contractors.value.filter((contractor) =>
    `${contractor.businessName || ""} ${contractor.email || ""}`
      .toLowerCase()
      .includes(term)
  );
});
