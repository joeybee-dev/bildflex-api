        ...
        <div class="col-12 col-md-9 col-lg-7">
          <div class="card auth-card border-0 shadow-sm">
            <div class="auth-header text-center">
              <h2 class="auth-title mb-2">Register Contractor</h2>
              <p class="auth-subtitle mb-0">
                Create your contractor account and showcase your services.
              </p>
            </div>

            <div class="card-body p-4 p-md-5">
              <form @submit.prevent="registerContractor">
                <div class="row g-3">
                  ...
                  <div class="col-12 col-md-6">
                    ...
                    <input v-model.trim="form.specialization" type="text" class="form-control" placeholder="e.g. General Construction" />
                  </div>
              ...
              <p class="text-center auth-footer-text mt-4 mb-0">
                ...
                <router-link to="/contractor/login" class="auth-link">Login here</router-link>
              </p>
            </div>
          </div>
        </div>
...
const registerContractor = async () => {
  ...
  try {
    ...
    const response = await api.post("/contractors/register", payload);
    notyf.success(response.data?.message || "Contractor registered successfully.");
    router.push("/contractor/login");
  } catch (err) {
    notyf.error(err.response?.data?.error || "Failed to register contractor.");
  } finally {
    loading.value = false;
  }
};
