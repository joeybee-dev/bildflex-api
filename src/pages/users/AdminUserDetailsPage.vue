    ...
    <div class="container">
      <div class="card auth-card border-0 shadow-sm">
        <div class="auth-header text-center">
          <h2 class="auth-title mb-2">Admin User Details</h2>
          <p class="auth-subtitle mb-0">Review user account information.</p>
        </div>
        ...
        <div class="card-body p-4 p-md-5">
          ...
          <div v-else class="row g-4">
            ...
            <div class="col-12">
              <router-link to="/admin/users" class="btn login-btn">Back to Admin Users</router-link>
            </div>
          </div>
        </div>
      </div>  
...
const fetchUser = async () => {
  try {
    ...
    Object.assign(user, response.data?.user || response.data || {});
  } catch (err) {
    console.error(err);
